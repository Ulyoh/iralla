...
gmap.Marker.prototype.setAddingConnections = function(){
	...
	if (typeof(BusStation.previousModified) != 'undefined') {
		...
		var size = SubMap._busStationArrays.sizeForAZoomValue[map.getZoom()-1];
	...
	if ((typeof(SubMap._busLinesArray) != 'undefined') && (SubMap._busLinesArray[0] != "")) {
		...
		for (var i = 0; i < SubMap._busLinesArray.length; i++) {
			if((typeof(SubMap._busLinesArray[i].listeners)!= 'undefined') && (typeof(SubMap._busLinesArray[i].listeners['click'])!= 'undefined')){
		...
		for (i = 0; i < SubMap._busLinesArray.length; i++) {
			...
			SubMap._busLinesArray[i].idOfListenerOfAddConnection = SubMap._busLinesArray[i].addFunctionsToListener('click', Connection, ["", this, SubMap._busLinesArray[i], "eVeNt:MouseEvent.latLng"]);
		}
	}
	//found the index of the bus station in arrayofbusstation
	for( i = 0; i < SubMap._busStationArrays.length; i++){
		if (SubMap._busStationArrays[i] == this)
			break;
	}
	...
	document.getElementById('button_remove_BusStation').setAttribute('onclick', 'SubMap._busStationArrays[' + i + "].remove('linkBusStationToTroncales');");
	...
	//document.getElementById('button_increase_polygonCircle').setAttribute('onclick', 'SubMap._busStationArrays[' + i + '].polygonCircle.increaseRadius();');
	document.getElementById('button_increase_polygonCircle').setAttribute('onclick', 'SubMap._busStationArrays[' + i + '].circle.increaseRadius();');
	...
	//document.getElementById('button_decrease_polygonCircle').setAttribute('onclick', 'SubMap._busStationArrays[' + i + '].polygonCircle.decreaseRadius();');
	document.getElementById('button_decrease_polygonCircle').setAttribute('onclick', 'SubMap._busStationArrays[' + i + '].circle.decreaseRadius();');
	...
	document.getElementById('button_reset_connections').setAttribute('onclick', 'SubMap._busStationArrays[' + i + '].createConnectionsWithAllBusLines();');
	...
	document.getElementById('button_deselect').setAttribute('onclick', 'SubMap._busStationArrays[' + i + '].deselect();');
	...
	document.getElementById('div_set_name').innerHTML = "name of the selected bus station :<br/> <textarea rows='2' cols='40' id='busStationName'  onKeyUp=SubMap._busStationArrays[" + i + "].setNewNameToShow() /> " + 
	this.name + "</textarea>";
...
gmap.Marker.prototype.createConnectionsWithAllBusLines = function(){
	...
	for(var j = 0; j < SubMap._busLinesArray.length; j++){
		...
		((SubMap._busLinesArray[j].type == "mainLine") || (SubMap._busLinesArray[j].type == "feeder"))){
			
			this.createConnectionsWithOneBusLine(SubMap._busLinesArray[j]);
			
		}
		//if making connections with feeders and others:
		else if((preTreatment.current == 'makeVirtualsBusStation') && 
		(this.type == 'virtual') &&
		((SubMap._busLinesArray[j].type == "feeder") || (SubMap._busLinesArray[j].type == "other"))){
			this.createConnectionsWithOneBusLine(SubMap._busLinesArray[j]);
		}
		...
		(SubMap._busLinesArray[j].type == "virtual")){
...
gmap.Marker.prototype.createConnectionsWithAllBusLines_OneStep = function(index, busStationIndex){
	...
	if((SubMap._busLinesArray[index].type == "mainLine") || (SubMap._busLinesArray[index].type == "feeder")){
		this.createConnectionsWithOneBusLine(SubMap._busLinesArray[index]);
	}

	index++;
	getAddInfoDiv().innerHTML = index + '/' + SubMap._busLinesArray.length + 'bus lines checked';

	if ( index < SubMap._busLinesArray.length ){
		setTimeout(function(){SubMap._busStationArrays[' + busStationIndex + '].createConnectionsWithAllBusLines_OneStep(' + index + ',' + busStationIndex + ')}, 100);
	}
...
//	resultIntersection = busLine.isPolygonCircleCrossed(this.polygonCircle);
	...
	for(var i = 0; i < SubMap._busLinesArray.length; i++){
		if( SubMap._busLinesArray[i] != busLine){
...
gmap.Marker.prototype.remove = function(whichMenuOpen){
	...
	//remove it from the list of SubMap._busStationArray
	...
	for( var i = 0; i < SubMap._busStationArrays.length; i++){	//TODO hide arrayofbusstations make it accessible only in map
		if(SubMap._busStationArrays[i] == this ){
			SubMap._busStationArrays.splice(i, 1);
			break;
		}
	}
