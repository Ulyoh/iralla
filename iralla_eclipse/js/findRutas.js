...
function showChoosenRuta(){
	var busLine = SubMap._busLinesArray.getItemById(this.rutasInfos.id);
	if (busLine === false) {
		var array = [];
		array.push(this.rutasInfos);
		map.addBusLinesFromDb(array);
		busLine = SubMap._busLinesArray.getItemById(this.rutasInfos.id);
	}
	else {
		busLine.setMap(map);
	}
 ...
 *			//remove all the roads already shown:
			...
			SubMap._busLinesArray.removeOnePolylineFromId(id);
 * 
 * 
 * 
 */