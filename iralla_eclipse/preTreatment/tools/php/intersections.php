...
function inter_segments(array $segments) {
	...
	foreach ( $segments as $segment ) {
		$endpoints_to_treat = array_merge( $endpoints_to_treat, $segment->get_pts_as_array() );
	}
	...
	usort( $endpoints_to_treat, "cmp_local" );
	...
	//$last_maillon = $new_maillon;
				...
				treat_intersection_possibility( $below_seg, $seg_s_event, &$endpoints_to_treat );
				treat_intersection_possibility( $seg_s_event, $above_seg, &$endpoints_to_treat );
			}
		}
		elseif (isset( $event->position ) && ($event->position == 'right')) {
			...
			//get the above and below segment:
			$above_seg = ($seg_s_event->maillon->next != null) ? $seg_s_event->maillon->next->segment : null; //TODO verify if error of selection above below?????
			$below_seg = get_below_segment( $new_maillon );
			...
			treat_intersection_possibility( $below_seg, $above_seg, &$endpoints_to_treat );
		}
			...
			$below_seg = ($seg1->maillon->previous != null) ? $seg1->maillon->previous->segment : null;
			$above_seg = ($seg1->maillon->next != null) ? $seg1->maillon->next->segment : null;
			
			//swap theirs positions:
			$seg1->maillon->swap_with( $seg2->maillon, &$sweep_line_beginning);
			
			treat_intersection_possibility( $below_seg, $seg1, &$endpoints_to_treat );
			treat_intersection_possibility( $seg2, $above_seg, &$endpoints_to_treat );
		}
	}
	return $output_list;
}
...
function insert_intersect_point_in_array($intersect_result, $endpoints_to_treat) {
	...
	//$endpoints_to_treat[$offset] = $intersect_result[1];/////////////////TO DO
	array_splice($endpoints_to_treat, $offset, 0, "empty");
