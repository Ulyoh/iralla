 ...
 */
 ...
 gmap.Polyline.prototype.setAddingConnections = function(){
	...
	for( i = 0; i < SubMap._busStationArray.length; i++){
		...
		SubMap._busStationArray[i].idOfListenerOfAddConnection = SubMap._busStationArray[i].addFunctionsToListener('click',SubMap._busStationArray[i].createConnectionsWithOneBusLine,[SubMap._busStationArray[i], this]);
	...
	document.getElementById('button_create_connections').setAttribute('onclick', 'SubMap._busStationArray[' + i + '].createConnectionsWithAllBusStations();');
	...
	document.getElementById('div_set_name').innerHTML = "name of the selected bus line :<br/> <textarea rows='2' cols='40' id='busLineName'  onKeyUp=SubMap._busStationArray[" + i + "].setNewNameToShow() />" + 
	this.name + "</textarea>";
 ...
 gmap.Polyline.prototype.createConnectionsWithAllBusStations = function(){
	...
	for( var i = 0; i < SubMap._busStationArray.length; i++){
		SubMap._busStationArray[i].createConnectionsWithOneBusLine(this);
	}
 };
