...
gmap.Marker.prototype.setAddingConnections = function(){
	...
	for( i = 0; i < SubMap._busStationsArray.length; i++){
		if (SubMap._busStationsArray[i] == this)
			break;
	}
	...
	document.getElementById('button_remove_BusStation').setAttribute('onclick', 'SubMap._busStationsArray[' + i + "].remove('linkBusStationToTroncales');");
	...
	//document.getElementById('button_increase_polygonCircle').setAttribute('onclick', 'SubMap._busStationsArray[' + i + '].polygonCircle.increaseRadius();');
	document.getElementById('button_increase_polygonCircle').setAttribute('onclick', 'SubMap._busStationsArray[' + i + '].circle.increaseRadius();');
	...
	//document.getElementById('button_decrease_polygonCircle').setAttribute('onclick', 'SubMap._busStationsArray[' + i + '].polygonCircle.decreaseRadius();');
	document.getElementById('button_decrease_polygonCircle').setAttribute('onclick', 'SubMap._busStationsArray[' + i + '].circle.decreaseRadius();');
	...
	document.getElementById('button_reset_connections').setAttribute('onclick', 'SubMap._busStationsArray[' + i + '].createConnectionsWithAllBusLines();');
	...
	document.getElementById('button_deselect').setAttribute('onclick', 'SubMap._busStationsArray[' + i + '].deselect();');
	...
	document.getElementById('div_set_name').innerHTML = "name of the selected bus station :<br/> <textarea rows='2' cols='40' id='busStationName'  onKeyUp=SubMap._busStationsArray[" + i + "].setNewNameToShow() /> " + 
	this.name + "</textarea>";
...
gmap.Marker.prototype.createConnectionsWithAllBusLines_OneStep = function(index, busStationIndex){
	...
	if ( index < SubMap._busLinesArray.length ){
		setTimeout(function(){SubMap._busStationsArray[' + busStationIndex + '].createConnectionsWithAllBusLines_OneStep(' + index + ',' + busStationIndex + ')}, 100);
	}
...
gmap.Marker.prototype.remove = function(whichMenuOpen){
	...
	for( var i = 0; i < SubMap._busStationsArray.length; i++){	//TODO hide arrayofbusstations make it accessible only in map
		if(SubMap._busStationsArray[i] == this ){
			SubMap._busStationsArray.splice(i, 1);
			break;
		}
	}
